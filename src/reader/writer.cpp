...
void
Writer::write_raw(const std::string& name, const std::string& value)
{
  indent();
  *out << '(' << name << " ";
  write_escaped_string(value, false);
  *out << ")\n";
}

void
Writer::write_escaped_string(const std::string& str, bool brackets)
{
  if (brackets) *out << '"';
