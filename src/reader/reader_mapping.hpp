...
public:
  ...
  bool get_custom(const char* key, C& value, F from_string) const
  {
    std::string text;
    if (!get(key, text))
    {
  ...
  /** Compatibility with old reader code */
  ...
  bool read_int_vector(const char* key, std::vector<int>* value) const;
  bool read_string_vector(const char* key, std::vector<std::string>* value) const;
