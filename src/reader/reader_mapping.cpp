...
bool
ReaderMapping::read_int_vector(const char* key, std::vector<int>* value) const
{
  return get(key, *value);
}

bool
ReaderMapping::read_string_vector(const char* key, std::vector<std::string>* value) const
{
  return get(key, *value);
}
